...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const Model = QueryLog as any
    const [items, total] = await Promise.all([
      Model.find(filter).sort({ createdAt: -1 }).skip((page-1)*limit).limit(limit).select('-explanation -schema').lean(),
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    const db = await connectDB()
    if (db) await (QueryLog as any).findByIdAndDelete(id)
